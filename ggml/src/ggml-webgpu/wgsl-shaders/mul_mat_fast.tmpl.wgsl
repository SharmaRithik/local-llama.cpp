...
fn store_val(acc: array<array<f32, TILE_Y>, TILE_X>, tx: u32, ty: u32) -> vec4<f32> {
    return vec4<f32>(acc[tx][ty], f32(acc[tx][ty + 1]), f32(acc[tx][ty + 2]), f32(acc[tx][ty + 3]));
}
...
var<workgroup> src0_shmem: array<{{SRC0_TYPE}}, (WORKGROUP_SIZE_Y * TILE_Y * TILE_K)/{{VEC_SIZE}}>;
var<workgroup> src1_shmem: array<{{SRC1_TYPE}}, (WORKGROUP_SIZE_X * TILE_X * TILE_K)/{{VEC_SIZE}}>;
